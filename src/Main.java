...
public class Main {
	...
	public static void main(String[] args){
		...
		/* set our arrays to -1 to indicate 
		    no reads or writes performed yet */
		for (int i = 0; i < 8; i++) {  
			lastReads.add(-1);
			lastWrites.add(-1);
			registerNames[i] = i + 1;
		...
		/* set our state table to -1 to indicate
		    nothing had been read or written to */
		...
		try {
			...
			int lineNum = 0;
			while(grabbedLine != null) { // looping while we have instructions 
				
				// split up line - process instruction
